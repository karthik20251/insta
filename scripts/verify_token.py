...
def main() -> int:
    ...
    if not data.get("is_valid"):
        print(f"  ✗ Token is INVALID: {r.json()}"); return 1
    ...
    if missing:
        print(f"  ⚠ Missing required permissions: {missing}")

    step(2, "Listing your Facebook Pages...")
    r = requests.get(f"{GRAPH}/me/accounts", params={"access_token": token}, timeout=30)
    pages = r.json().get("data", [])
    if not pages:
        print("  ✗ No pages found. You need a Facebook Page linked to your IG."); return 1
    for p in pages:
    ...
    for p in pages:
        ...
        if ig:
            print(f"  ✓ Page '{p['name']}' → IG_USER_ID={ig['id']}")
    ...
    if user_id not in found_ig:
        print(f"\n⚠ The IG_USER_ID in .env ({user_id}) is NOT one of the linked accounts above.")
        return 1
    ...
    print("\n✅ All good. You can now post for real (remove --dry-run).")
    return 0
